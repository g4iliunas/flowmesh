...
uv_tcp_t *fm_manager_server_init(uv_loop_t *loop, const char *address,
                                 const uint16_t port)
{
    uv_tcp_t *manager_server = (uv_tcp_t *)malloc(sizeof(uv_tcp_t));
    if (!manager_server)
        return NULL;

    uv_tcp_init(loop, manager_server);
    struct sockaddr_in sin = {0};
    uv_ip4_addr(address, port, &sin);
    uv_tcp_bind(manager_server, (struct sockaddr *)&sin, 0);

    if (uv_listen((uv_stream_t *)manager_server, SOMAXCONN,
                  on_new_connection) != 0) {
        free(manager_server);
        return NULL;
    }
